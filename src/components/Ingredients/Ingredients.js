...
const Ingredients = () => {
  ...
  const removeIngredientsHandler = (ingredientId) => {
    dispatchHttp({type: 'SEND'});
    fetch(`https://react-ingredients-app-51469-default-rtdb.asia-southeast1.firebasedatabase.app/ingredients/${ingredientId}.json`, 
    {
      method: 'DELETE',
    }
    ).then(response => {
      ...
      // );
      disapatch({type: 'DELETE', id: ingredientId
    });
    }).catch(error => {
      dispatchHttp({type: 'ERROR', errorMessage: 'Somethong went wrong!' });
    });
  };
